...
from utils import is_password_strong, is_valid_phone_number
...
def register():
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                "INSERT INTO users (username, phone_number, password) VALUES (%s, %s, %s)",
...
def forgot_password():
    if request.method == 'POST':
        ...
        try:
            ...
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT id FROM users WHERE username = %s AND phone_number = %s",
...
def reset_password():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            with conn.cursor() as cursor:
                cursor.execute(
                    "UPDATE users SET password = %s WHERE phone_number = %s",
